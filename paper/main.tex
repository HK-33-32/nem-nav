...
\usepackage{enumitem}
\usepackage{graphicx}
\IfFileExists{paper/figures/architecture.pdf}{%
...
\newcommand{\paperinput}[1]{%
  \IfFileExists{paper/#1}{\input{paper/#1}}{\input{#1}}%
}
...
\author{
\IEEEauthorblockN{Peshkov Andrei}
\texttt{anpeshkov203@gmail.com}}
...
and 5 seeds, we find that episodic memory is the dominant driver of revisit performance, closing the gap between
...
either large vision--language models~\cite{majumdar2022zson, gadre2023cows}
or instruction-tuned planners~\cite{zhou2023esc, dorbala2023llmnav};
...
exploration~\cite{yamauchi1997frontier} and a goal classifier; recent
zero-shot systems replace the classifier with vision--language
models~\cite{majumdar2022zson, gadre2023cows, zhou2023esc} or query
LLMs for high-level subgoals~\cite{dorbala2023llmnav,
...
caches~\cite{fang2019smt, fukushima2022ohm} to FAISS-backed retrieval
buffers~\cite{khandelwal2022clipnav, savinov2018sptm}. Our memory uses
...
\cite{chaplot2020topological, savinov2018sptm}; here we use a simple
...
Prompted-LLM planners are now standard~\cite{zhou2023esc,
...
loads a frozen ViT-B/32~\cite{radford2021clip} with a one-line code change. All downstream
modules consume only the unit-vector outputs.

\subsection{Episodic Memory and Retrieval}

Memory $\mathcal{M}$ is a FAISS~\cite{johnson2019faiss} \texttt{IndexFlatIP} (cosine similarity
...
\textbf{Why a controlled testbed?}  Replacing Habitat-Sim~\cite{savva2019habitat} with a small
...
\begin{figure}[t]
\centering
\includegraphics[width=0.45\linewidth]{scene_trajectory.pdf}
...
(SR), SPL~\cite{anderson2018evaluation}, mean distance to goal at
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.95\linewidth]{ci_bars.pdf}
\caption{Headline ablation bars with 95\,\% confidence intervals over 5
seeds. The left panel shows success rate, and the right panel shows the
per-mode SPL summary used throughout the main results. Markers above bars
follow the paired Wilcoxon signed-rank test against the \textbf{Frontier}
baseline on per-seed SPL ($^{*}p\!<\!0.05$, $^{**}p\!<\!0.01$,
$^{\dagger}p\!<\!0.001$). Memory-enabled modes saturate near
$\mathrm{SR}\approx 0.98$ and $\mathrm{SPL}\approx 0.83$, while the
memory-free baselines remain far below.}
\label{fig:cibars}
\end{figure*}
...
\label{tab:main}
\resizebox{\columnwidth}{!}{\paperinput{tables/main_results.tex}}
\end{table}
...
\label{tab:fe-vs-rv}
\footnotesize
\resizebox{\columnwidth}{!}{\paperinput{tables/first_vs_revisit.tex}}
\end{table}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.7\linewidth]{learning_curve_grid.pdf}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.95\linewidth]{per_goal_class.pdf}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.31\linewidth]{learning_curve.pdf}
\hfill
\includegraphics[width=0.31\linewidth]{memory_budget_sweep.pdf}
\hfill
\includegraphics[width=0.31\linewidth]{topk_sweep.pdf}
...
\small
\resizebox{\columnwidth}{!}{\paperinput{tables/scene_size_sweep.tex}}
\end{table}
...
\label{tab:failures}
\small
\resizebox{\columnwidth}{!}{\paperinput{tables/failure_taxonomy.tex}}
\end{table}

\begin{figure}[t]
\centering
\includegraphics[width=0.6\linewidth]{failure_taxonomy.pdf}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.95\linewidth]{gallery_overview.pdf}
...
\noindent \\pip install -e ".[dev]"\\
python -m nem\_nav.scripts.run\_experiment\_matrix \textbackslash{}\\
\hspace*{1.5em}--run\_id main --seeds 0 1 2 3 4 \textbackslash{}\\
\hspace*{1.5em}--n\_scenes 5 --episodes\_per\_scene 15 --max\_steps 200\\
python -m nem\_nav.scripts.generate\_artifacts --run\_id main\\
python -m nem\_nav.scripts.failure\_taxonomy --run\_id main\\
python -m nem\_nav.scripts.qualitative\_gallery --seeds 0 1 2\par
}
